...
{
  // define FCB_BUF to decompress the config into it.
#ifdef FCB_BUF
extern uint32_t FCB_BUF[];
uint32_t *fcb_ptr = FCB_BUF;
#define FCB_CONFIG (*fcb_ptr++)
#else
#define FCB_CONFIG (FCB->AUTO)
#endif
...
#define MIN_INDEX   (MAX_DATA + 3)
  ...
  FCB->CTRL = FCB_CTRL_AUTO;
#endif
  FCB_CONFIG = *(uint32_t *)(config_addr + 0); // IDCODE
  FCB_CONFIG = *(uint32_t *)(config_addr + 4); // USERID
...
{
  uint32_t pin_bit = FCB_PIN_BIT(pin_id);
  uint8_t *io_data = io_cfg->BYTES + (pin_bit / 8);
  uint8_t *byte_data = pin_cfg->BYTES;
  for (int pin_offset = pin_bit % 8 + 10; pin_offset > 0; pin_offset -= 8) {
    *byte_data++ = UTIL_ReverseBits(*io_data++);
  }
  pin_cfg->WORD >>= (pin_bit % 8);
}
...
void FCB_SetPinConfig(FCB_IO_TypeDef *io_cfg, int pin_id, FCB_PIN_TypeDef *pin_cfg)
{
  uint32_t pin_bit = FCB_PIN_BIT(pin_id);
  uint8_t *io_data = io_cfg->BYTES + (pin_bit / 8);
  uint32_t pin_mask = 0x3ff << (pin_bit % 8);
  uint32_t pin_word = (pin_cfg->WORD << (pin_bit % 8)) & pin_mask;
  uint8_t *byte_data = (uint8_t *)&pin_word;
  uint8_t *byte_mask = (uint8_t *)&pin_mask;
  for (int pin_offset = pin_bit % 8 + 10; pin_offset > 0; pin_offset -= 8) {
    MODIFY_BIT(*io_data, UTIL_ReverseBits(*byte_mask++), UTIL_ReverseBits(*byte_data++));
    ++io_data;
  }
}
...
void FCB_WritePLLConfig(FCB_PLL_Typedef *pll_cfg)
{
  ...
  for (int i = 0; i < FCB_PLL_WORDS; ++i) {
    ...
    for (int j = i * 4; j < i * 4 + 4; ++j) {
      *ptr++ = UTIL_ReverseBits(pll_cfg->BYTES[j]);
    }
    FCB0->DATA = word;
  }
  FCB->CTRL = 0;
