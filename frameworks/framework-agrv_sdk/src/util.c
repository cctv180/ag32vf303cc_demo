...
bool UTIL_StartLogger()
{
  ...
  UART_Init(UARTx(LOGGER_UART), BAUD_RATE,
...
void UTIL_WriteLogger(const void *p, unsigned int num)
{
  UART_Send(MSG_UART, p, num);
}

unsigned int UTIL_ReadLogger(void *p, unsigned int num, unsigned int timeout)
{
  return UART_Receive(MSG_UART, p, num, timeout);
}
...
void UTIL_WriteLogger(const void *p, unsigned int num)
{
  SEGGER_RTT_Write(0, p, num);
}
unsigned int UTIL_ReadLogger(void *p, unsigned int num, unsigned int timeout)
{
  (void) timeout; // Not used.
  return SEGGER_RTT_Read(0, p, num);
}
...
__attribute__((weak)) void UTIL_WriteLogger(const void *p, unsigned int num)
{
}
__attribute__((weak)) unsigned int UTIL_ReadLogger(void *p, unsigned int num, unsigned int timeout)
{
  return 0;
}
