...
#define DMAC_MAX_TRANSFER_SIZE    ((1 << 12) - 4) // Use a number that can be divided by 4 in case src/dst widths differ
...
#define DMAC_TRANSFER_SIZE_MASK ((1 << 12) - 1)
...
#define DMAC_LLI_COUNT(__transferSize) ((__transferSize + DMAC_MAX_TRANSFER_SIZE - 1) / DMAC_MAX_TRANSFER_SIZE)
...
static inline uint32_t DMAC_GetTransferSize(DMAC_ChannelNumTypeDef channel) { return DMAC_GetControl(channel) & DMAC_TRANSFER_SIZE_MASK; }
static inline void     DMAC_SetTransferSize(DMAC_ChannelNumTypeDef channel, uint32_t transferSize)
{
  MODIFY_BIT(DMAC->Channels[channel].Control, DMAC_TRANSFER_SIZE_MASK, transferSize << DMAC_TRANSFER_SIZE_OFFSET);
}
