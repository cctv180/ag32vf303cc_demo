...
void UART_SetBaudRateFreq(UART_TypeDef *uart, UART_BaudRateTypeDef baudrate, uint32_t freq)
{
  uint32_t ibrd  = freq / baudrate / 16;
  uint32_t fbrd = freq - ibrd * baudrate * 16;
  fbrd = (fbrd * 4 + baudrate / 2) / baudrate;
  uart->IBRD = ibrd;
  uart->FBRD = fbrd;
}
...
{
  UART_SetBaudRateFreq(uart, baudrate, SYS_GetPclkFreq());
}
...
ReturnCodeTypedef UART_SendStr(UART_TypeDef *uart, const char *str)
{
  return UART_Send(uart, (const unsigned char *)str, strlen(str));
}
...
unsigned int UART_Receive(UART_TypeDef *uart, unsigned char *p, unsigned int num, unsigned int timeout)
{
  ...
  while (cnt < num) {
    if (UART_IsRxFifoEmpty(uart)) {
      ...
      if (timeout == 0 || UTIL_GetTick() - tick > timeout) {
        break;
      } else {
        continue;
      }
