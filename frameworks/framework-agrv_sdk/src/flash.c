...
void FLASH_ProgramOptionHalfWord(uint32_t addr, uint16_t data)
{
...
{
  FLASH_ProgramOptionHalfWord(addr, ~(uint16_t)data << 8 | data);
}
...
{
  FLASH_ProgramOptionHalfWord((uint32_t)&FLASH_OB->RDP, 0); // Writing any other other value will trigger a full chip erase
}

void FLASH_DisableReadProtect(void)
{
  FLASH_ProgramOptionHalfWord((uint32_t)&FLASH_OB->RDP, RDP_WORD); // Will trigger a full chip erase
}
...
{
  uint32_t rdid = FLASH_FlexRead(0x9F, 0, 0, 3, 0);
  return 1 << ((rdid >> 16) & 0xff); // In bytes
}
