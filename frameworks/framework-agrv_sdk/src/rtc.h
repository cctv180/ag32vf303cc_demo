...
static inline void RTC_Enable(void)    { RTC_MODIFY_BIT(RTC->BDCR, 0, RTC_BDCR_RTCEN); }
...
static inline void RTC_ClearInt(uint16_t bits)
{
  while (RTC_GetDivider() == RTC_GetPrescaler());
  RTC_ClearFlag(bits);
}
