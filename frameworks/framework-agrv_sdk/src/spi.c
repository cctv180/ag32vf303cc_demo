...
void SPI_SendDMAStart(SPI_TypeDef *spi, int tx_bytes, const uint32_t *tx_data, DMAC_ChannelNumTypeDef channel)
{
  // If tx_bytes is no more than 4, use SPI_Send.
  ASSERT(tx_bytes > 4 && tx_bytes <= SPI_PHASE_BYTE_CNT_MAX);
  SPI_SetPhaseCtrl(spi, SPI_PHASE_0, SPI_PHASE_ACTION_TX, SPI_PHASE_MODE_SINGLE, 4);
  SPI_SetPhaseData(spi, SPI_PHASE_0, *tx_data);
  SPI_SetPhaseCtrl(spi, SPI_PHASE_1, SPI_PHASE_ACTION_TX, SPI_PHASE_MODE_SINGLE, tx_bytes - 4);
  DMAC_Config(channel, (uint32_t)(tx_data + 1), (uint32_t)&spi->PHASE_DATA[SPI_PHASE_1],
...
void SPI_DMAWait(SPI_TypeDef *spi, DMAC_ChannelNumTypeDef channel)
{
  SPI_WaitForDone(spi);
...
{
  SPI_SendDMAStart(spi, tx_bytes, tx_data, channel);
  SPI_DMAWait(spi, channel);
}
...
void SPI_SendAndReceiveDMAStart(SPI_TypeDef *spi, int tx_bytes, const uint32_t *tx_data,
...
void SPI_SendAndReceiveDMA(SPI_TypeDef *spi, int tx_bytes, const uint32_t *tx_data,
                           int rx_bytes, uint32_t *rx_data, DMAC_ChannelNumTypeDef channel)
{
  SPI_SendAndReceiveDMAStart(spi, tx_bytes, tx_data, rx_bytes, rx_data, channel);
  SPI_DMAWait(spi, channel);
}
...
{
  // Works for most vendors. Much easier than reading SFDP.
  uint32_t rdid = SPI_FLASH_ReadID(spi);
  return 8 << ((rdid >> 16) & 0xff); // In bits
}
