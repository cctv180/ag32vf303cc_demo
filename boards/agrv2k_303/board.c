...
#ifdef BOARD_OSC_CALIBRATION
#ifndef AGM_LOGIC_COMPRESS
  // Apply calibrated OSC value for non-compressed logic. It's already done automatically for compressed logic.
  PERIPHERAL_ENABLE(FCB, 0);
  uint16_t osc_cal = FLASH_OB->OSC_CAL;
  uint8_t *osc_val = (uint8_t *)&osc_cal;
  FCB_OSC_TypeDef osc_cfg;
  FCB_IO_TypeDef io_cfg;
  FCB_ReadIOConfig(&io_cfg);
  FCB_GetOscConfig(&io_cfg, &osc_cfg);
  osc_cfg.CFG_RESSEL   = 0;
  osc_cfg.CFG_RCOSCCAL = osc_val[0] + osc_val[1] == 0xff ? osc_val[0] : 87;
  osc_cfg.CFG_RCOSC_EN = 1;
